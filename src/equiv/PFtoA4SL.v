...
Proof.
  ...
  - (* read *)
    ...
    guardH READ_SPEC. clear STATE LOCAL0 H.
      ...
      { ii. des.
        ...
        - inv EID. inv REL. des. inv H. inv H2.
          ...
          + inv H. exploit RF2; eauto. i. des.
            ...
            rewrite R in x6. inv x6. rewrite e. ss.
    ...
    + destruct ex0; ss. econs. s. splits.
      ...
      * econs.
        { ii. inv EID. destruct eid as (tid1, eid1).
          ...
          unfold v_gen. ss. rewrite <- H6. rewrite x2.
          exploit EX2.(XR); eauto.
          - instantiate (1 := ALocal.next_eid alc1).
            ss. rewrite List.app_length. ss. unfold ALocal.next_eid. clear. lia.
          - condtac; cycle 1.
            { rewrite Nat.eqb_neq in X. ss. }
            move READ_SPEC at bottom. desH READ_SPEC. revert COH1.
            rewrite fun_add_spec. condtac; cycle 1.
            { exfalso. apply c. ss. }
            i. unfold ALocal.next_eid in *. rewrite R in x6. inv x6.
            rewrite COH1 at 2. unfold join. ss. }
        { exploit sim_trace_sim_th; try exact SIMTR; eauto. intro L2. ss.
          ...
          { rewrite Nat.eqb_neq in Heq. ss. }
          move READ_SPEC at bottom. unguardH READ_SPEC. des. revert COH1.
          rewrite fun_add_spec. condtac; cycle 1.
          { exfalso. apply c. ss. }
          i. des. unguardH x3. des.
          - econs 1. rewrite <- x3. auto.
          - exploit sim_traces_memory; eauto. i. des.
            ...
            { instantiate (1 := (tid', eid)). subst.
              ...
              exploit sim_trace_last; try exact REL7. i. des. subst.
            ...
            subst. unfold v_gen. ss. rewrite <- H6. rewrite x10.
            rewrite COH1 at 1. ss.
        }
    ...
    + intro loc. specialize (COH_CL loc). des.
      ...
      { exists mloc_cl. splits; ss.
        ...
        - rewrite sim_local_coh_cl_step. rewrite inverse_step.
          ...
          inv VLOC. rewrite VAL0 in *. rewrite H_CL in *. ss.
      }
      destruct (lt_eq_lt_dec
          (join
            (join (View.ts (ValA.annot (sem_expr rmap1 eloc)))
                (join (View.ts (Local.vrn lc1))
                  (join (View.ts (ifc (OrdR.ge ord OrdR.acquire) (Local.vrel lc1)))
                      bot)))
            (View.ts
                (FwdItem.read_view
                  (Local.fwdbank lc1 (ValA.val (sem_expr rmap1 eloc))) ts ord)))
          (View.ts (Local.coh lc1 mloc_cl))).
      { (* <= *)
        exists mloc_cl. splits; ss.
        - i. funtac.
          + exploit (COH_MAX_CL (ValA.val (sem_expr rmap1 eloc))).
            { eapply Loc.cl_trans; try exact CL; eauto. apply Loc.cl_sym. ss. }
            i. unfold join at 1. unfold Time.join at 1. inv s; lia.
        ...
        - rewrite sim_local_coh_cl_step. rewrite inverse_step.
          ...
          + etrans; try eapply COH_CL; eauto.
            funtac. inversion e. rewrite <- join_l. ss.
          + inv EID. obtac.
            exploit EX2.(LABELS); eauto; ss.
            { rewrite List.app_length. s. lia. }
            rewrite List.nth_error_app2, Nat.sub_diag; ss. i. simplify. ss.
      ...
      { (* > *)
        ...
        - rewrite sim_local_coh_cl_step. rewrite inverse_step.
          ...
          ii. des; ss.
          + etrans; try eapply COH_CL; eauto.
            funtac. rewrite <- join_r. unfold le. lia.
          + inv EID. obtac.
            exploit EX2.(LABELS); eauto; ss.
            { rewrite List.app_length. s. lia. }
            rewrite List.nth_error_app2, Nat.sub_diag; ss. i. simplify. ss.
            ...
            destruct (length (ALocal.labels alc1) =? ALocal.next_eid alc1) eqn:Heq; cycle 1.
            { rewrite Nat.eqb_neq in Heq. ss. }
            funtac. unfold le. unfold join. unfold Time.join. lia.
      }
    ...
    + i. rewrite sim_local_lper_step. rewrite inverse_step.
      rewrite ? inverse_union. ii. des.
      * exploit LPER; eauto.
      * inv EID. obtac. destruct l; ss.
        exploit EX2.(LABELS); eauto; ss.
        { rewrite List.app_length. s. lia. }
        rewrite List.nth_error_app2, Nat.sub_diag; ss.
    + i. rewrite sim_local_per_step. rewrite inverse_step.
      rewrite ? inverse_union. ii. des.
      * exploit PER; eauto.
      * inv EID. obtac. destruct l; ss.
        exploit EX2.(LABELS); eauto; ss.
  ...
  - (* write *)
    ...
    { econs; ss. apply sim_rmap_add; try apply L.
      ...
      { ss. rewrite List.app_length. ss. unfold ALocal.next_eid. clear. lia. }
      destruct (ALocal.next_eid alc1 =? ALocal.next_eid alc1) eqn:Heq; ss.
      - rewrite fun_add_spec. condtac; ss. exfalso. apply c. ss.
      - rewrite Nat.eqb_neq in Heq. ss. }
    destruct L.(LC). ss. econs; ss.
    all: try rewrite List.app_length, Nat.add_1_r.
    + i. rewrite sim_local_coh_step. rewrite inverse_step.
    ...
    + intro loc. specialize (COH_CL loc). des.
      ...
      inv WRITABLE.
      destruct (classic (ts <= (View.ts (Local.coh lc1 mloc_cl)))).
      { (* <= *)
        exists mloc_cl. splits; ss.
        - i. funtac. inversion e. subst. lia.
        - rewrite sim_local_coh_cl_step. rewrite inverse_step.
          rewrite ? inverse_union.
          ii. des; ss.
          + etrans; try eapply COH_CL; eauto.
            funtac. inversion e. unfold le. lia.
          + inv EID. obtac.
            exploit EX2.(LABELS); eauto; ss.
            { rewrite List.app_length. s. lia. }
            rewrite List.nth_error_app2, Nat.sub_diag; ss. i. simplify. ss.
  ...
  - (* dmb *)
    ...
    + rewrite List.app_length, Nat.add_1_r.
      ...
      { rewrite <- join_r, <- join_r. etrans; try eapply VWO; eauto; ss.
        econs; eauto. econs. simtac.
      }
    + rewrite List.app_length, Nat.add_1_r.
      i. rewrite sim_local_lper_step. rewrite inverse_step.
  ...
  - (* dsb *)
    ...
    + rewrite List.app_length, Nat.add_1_r.
      i. rewrite sim_local_per_step. rewrite inverse_step.
      rewrite ? inverse_union. ii. des.
      * etrans; try eapply PER; eauto.
        condtac; ss. rewrite <- join_l. ss.
      * inv EID. obtac. destruct l; ss. eqvtac.
        exploit EX2.(LABELS); eauto; ss.
        { rewrite List.app_length. s. lia. }
        rewrite List.nth_error_app2, Nat.sub_diag; ss. intro Z. inv Z. ss.
        destruct rr; destruct rw; destruct wr; destruct ww; ss. cleartriv.
        rewrite <- join_r. etrans; try eapply LPER; eauto; ss.
  - (* if *)
  ...
  - (* flushopt *)
    ...
    + rewrite List.app_length, Nat.add_1_r.
      ...
      * inv EID. inv REL. obtac.
        rewrite <- join_l.
        inv L. inv LC. ss. apply LPER0. econs; eauto. econs. simtac.
      * inv EID. obtac.
        exploit EX2.(LABELS); eauto; ss.
        { rewrite List.app_length. s. lia. }
        rewrite List.nth_error_app2, Nat.sub_diag; ss. i. inv x0.
        ...
        inv LOCAL0. exploit LPERCL; eauto. intro Z. rewrite Z. ss.
    + rewrite List.app_length, Nat.add_1_r.
      i. rewrite sim_local_per_step. rewrite inverse_step.
      rewrite ? inverse_union. ii. des.
      * exploit PER; eauto.
